...
void FUnrealSharpEditorModule::ExportSpecialStructInfo()
{
    for (auto Processed = ProcessStructInfo(); auto &[Module, Info] : Processed)
    {
        ...
        ExportStructData(Module, Info);
    }
}

void FUnrealSharpEditorModule::ExportStructData(FStringView ModuleName, const FStructLayoutInfo& Info)
{
    const auto Json = MakeShared<FJsonObject>();

    const auto StructsField = MakeShared<FJsonObject>();
    Json->SetObjectField("Structs", StructsField);

    const auto &[BlittableStructs, NativeConstructionRequired] = Info;
    TArray<TSharedPtr<FJsonValue>> BlittableTypes;
    for (auto& Struct : BlittableStructs)
    {
        const auto StructField = MakeShared<FJsonObject>();
        StructField->SetStringField("Name", Struct);
        BlittableTypes.Add(MakeShared<FJsonValueObject>(StructField));
    }
    StructsField->SetArrayField("BlittableTypes", BlittableTypes);

    TArray<TSharedPtr<FJsonValue>> NativeTypes;
    for (auto &NativeStruct : NativeConstructionRequired)
    {
        auto TranslatableType = MakeShared<FJsonObject>();
        TranslatableType->SetStringField("Name", NativeStruct.Name);
        TranslatableType->SetBoolField("HasDestructor", NativeStruct.HasDestructor);
        NativeTypes.Emplace(MakeShared<FJsonValueObject>(TranslatableType));
    }
    StructsField->SetArrayField("NativelyTranslatableTypes", NativeTypes);

    auto OutputPath = FPaths::Combine(FPaths::ProjectDir(), TEXT("Config"),
        FString::Format(TEXT("{0}.UnrealSharpTypes.json"), {ModuleName}));
...
TMap<FString, FStructLayoutInfo> FUnrealSharpEditorModule::ProcessStructInfo()
{
    TMap<FString, FStructLayoutInfo> Result;
    for (TObjectIterator<UScriptStruct> StructIt; StructIt; ++StructIt)
    {
        const UScriptStruct* Struct = *StructIt;
        const auto CppOpts = Struct->GetCppStructOps();
        if (CppOpts == nullptr)
        {
            continue;
        }
        
        auto &[BlittableTypes, NativeTypes] = Result.FindOrAdd(Struct->GetPackage()->GetName());
        if (auto [ExpectedSize, MaxAlignment] = GetStructExpectedSize(Struct); ExpectedSize != CppOpts->GetSize())
        {
            NativeTypes.Emplace(Struct->GetStructCPPName(), CppOpts->HasDestructor());
        }
        else if (IsStructBlittable(Struct))
        {
            BlittableTypes.Emplace(Struct->GetStructCPPName());
        }
    }
...
TPair<int32, int32> FUnrealSharpEditorModule::GetStructExpectedSize(const UScriptStruct* Struct)
{
    int32 Size = 0;
    int32 MaxAlignment = 1;

    if (const auto Parent = Cast<UScriptStruct>(Struct->GetSuperStruct()); Parent != nullptr)
    {
        auto [ParentSize, ParentMaxAlignment] = GetStructExpectedSize(Parent);
        Size += ParentSize;
        MaxAlignment = FMath::Max(MaxAlignment, ParentMaxAlignment);
    }

    
    for (auto Property = Struct->PropertyLink; Property; Property = static_cast<FProperty*>(Property->Next))
    {
        if (const auto AsBoolProperty = CastField<FBoolProperty>(Property); AsBoolProperty != nullptr
            && AsBoolProperty->GetFieldMask() != 0xff && AsBoolProperty->GetFieldMask() != 0x1)
        {
            continue;
        }
        
        const int32 PropertyAlignment = Property->GetMinAlignment();
        // Update the maximum alignment requirement for the entire struct
        MaxAlignment = FMath::Max(MaxAlignment, PropertyAlignment);
    
        // Pad the current offset to match this property's alignment requirement
        Size = Align(Size, PropertyAlignment);
    
        // Add the property's size
        Size += Property->GetSize();
    }

    // Final size should be aligned to the maximum alignment requirement
...
bool FUnrealSharpEditorModule::IsStructBlittable(const UScriptStruct* Struct) {
    for (auto Property = Struct->PropertyLink; Property; Property = static_cast<FProperty*>(Property->Next))
    {
        if (const auto AsNumericProperty = CastField<FNumericProperty>(Property); AsNumericProperty == nullptr)
        {
            return false;
        }
    }
