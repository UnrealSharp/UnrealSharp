...
void MetaDataFromMap(const TMap<FName, FString>* PropertyMetaData, int32 IndentLevel)
{
	...
	{
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}(No Metadata Assigned)", Indent(IndentLevel + 1));
		return;
	}
	
	for (const TPair<FName, FString>& MetaDataPair : *PropertyMetaData)
	{
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- {1} = {2}", Indent(IndentLevel + 1), *MetaDataPair.Key.ToString(), *MetaDataPair.Value);
	}
}
...
{
#if WITH_EDITOR
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Metadata:", Indent(IndentLevel));
...
{
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Properties:", Indent(IndentLevel));
	...
	for (TFieldIterator<FProperty> It(Struct, EFieldIterationFlags::None); It; ++It)
	{
		...
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- {1} ({2}) ({3})", Indent(IndentLevel + 1), Property->GetName(), Property->GetClass()->GetName(), FString::Join(FlagStrings, TEXT(", ")));
		DumpMetaData(Property, IndentLevel + 2);
	}
	
	if (!bHasProperties)
	{
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}(No Properties Assigned)", Indent(IndentLevel + 1));
	}
}
...
{
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Tick Function:", Indent(IndentLevel + 1));
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- bCanEverTick: {1}", Indent(IndentLevel + 2), TickFunction.bCanEverTick ? TEXT("true") : TEXT("false"));
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- bStartWithTickEnabled: {1}", Indent(IndentLevel + 2), TickFunction.bStartWithTickEnabled ? TEXT("true") : TEXT("false"));
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- TickInterval: {1}", Indent(IndentLevel + 2), TickFunction.TickInterval);
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- TickGroup: {1}", Indent(IndentLevel + 2), static_cast<int32>(TickFunction.TickGroup));
}
...
void DumpDataAsClass(UClass* Class, int32 IndentLevel = 1)
{
	...
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Class Flags: ({1})", Indent(IndentLevel + 1), FString::Join(ClassFlagsStrings, TEXT(", ")));
	
	DumpPropertiesOfStruct(Class, IndentLevel + 1);

	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Functions:", Indent(IndentLevel + 1));
	...
	for (TFieldIterator<UFunction> It(Class, EFieldIterationFlags::None); It; ++It)
	{
		...
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- {1} ({2})", Indent(IndentLevel + 2), Function->GetName(), FString::Join(FunctionFlagsStrings, TEXT(", ")));
	...
	if (!bHasFunctions)
	{
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}(No Functions Assigned)", Indent(IndentLevel + 2));
	}
}

void DumpDataAsEnum(UEnum* Enum, int32 IndentLevel = 1)
{
	UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}Enum Values:", Indent(IndentLevel + 1));

	for (int32 i = 0; i < Enum->NumEnums(); i++)
	{
		UE_LOGFMT(LogUnrealSharpEditor, Log, "{0}- {1} = {2}", Indent(IndentLevel + 2), Enum->GetNameStringByIndex(i), Enum->GetValueByIndex(i));
	}
}
...
void DumpTypeReflectionData(const TArray<FString>& Args)
{
	if (Args.Num() != 1)
	{
		UE_LOG(LogUnrealSharpEditor, Warning, TEXT("Usage: UnrealSharp.DumpTypeReflectionData [TypeFullName]"));
		return;
	}
	...
	{
		UE_LOG(LogUnrealSharpEditor, Warning, TEXT("TypeFullName must be in the format 'Namespace.TypeName'"));
		return;
	}
...
static FAutoConsoleCommand CVarListTypesInAssembly(
	...
	FConsoleCommandWithArgsDelegate::CreateStatic([](const TArray<FString>& Args)
	{
		if (Args.Num() != 1)
		{
			UE_LOG(LogUnrealSharpEditor, Warning, TEXT("Usage: UnrealSharp.ListTypesInAssembly [AssemblyName]"));
			return;
		}
		...
		if (!IsValid(Assembly))
		{
			UE_LOGFMT(LogUnrealSharpEditor, Warning, "Assembly not found: {0}", *AssemblyName.ToString());
			return;
		}
		
		UE_LOGFMT(LogUnrealSharpEditor, Log, "Managed Types in Assembly {0}:", *AssemblyName.ToString());
