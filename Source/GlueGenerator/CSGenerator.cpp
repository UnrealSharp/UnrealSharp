...
void FCSGenerator::ExportClass(UClass* Class, FCSScriptBuilder& Builder)
{
	...
	FString thisNameSpace = BindingsModule.GetNamespace();
	Builder.DeclareDirective(BindingsModule.GetNamespace());
	Builder.GenerateScriptSkeleton(BindingsModule.GetNamespace(), true);
	
	AppendTooltip(Class, Builder);
	FString Abstract = Class->HasAnyClassFlags(CLASS_Abstract) ? "ClassFlags.Abstract" : "";

	bool IsHelperClass = false;
