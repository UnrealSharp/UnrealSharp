...
DECLARE_UNREALSHARP_EXPORTER(FStringExporter)
{
	void MarshalToNativeString(FString* NativeString, const char* ManagedString)
	{
		if (!NativeString)
		{
			return;
	...
	void MarshalToNativeStringView(FString* NativeString, const UTF16CHAR* ManagedString, int32 Length)
	{
		if (!NativeString)
		{
			return;
	...
	EXPORT_UNREALSHARP_FUNCTION(MarshalToNativeString)
	EXPORT_UNREALSHARP_FUNCTION(MarshalToNativeStringView)
}