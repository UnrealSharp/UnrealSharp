...
#define JSON_PARSE_OBJECT_ARRAY_CUSTOM_2(MemberName, SerializeAction, Optional) \
    bSuccess &= FCSMetaDataBase::ParseObjectArrayFieldCustom(MemberName, JsonObject, TEXT(#MemberName), Optional, SerializeAction);
#define JSON_PARSE_OBJECT_ARRAY_CUSTOM_3(CustomJson, MemberName, SerializeAction, Optional) \
    bSuccess &= FCSMetaDataBase::ParseObjectArrayFieldCustom(MemberName, CustomJson, TEXT(#MemberName), Optional, SerializeAction);
#define JSON_PARSE_OBJECT_ARRAY_CUSTOM(...) \
    _GET_MACRO_4(__VA_ARGS__, JSON_PARSE_OBJECT_ARRAY_CUSTOM_3, JSON_PARSE_OBJECT_ARRAY_CUSTOM_2)(__VA_ARGS__)

struct FCSMetaDataBase
{
	virtual ~FCSMetaDataBase() = default;
	...
	static bool ParseObjectArrayFieldCustom(
		TArray<T>& Destination,
		const TSharedPtr<FJsonObject>& Object,
		const FString& FieldName, bool bOptional,
		Fn&& SerializeAction)
	{
		const TArray<TSharedPtr<FJsonValue>>* FieldArray = nullptr;
		if (!Object->TryGetArrayField(FieldName, FieldArray) || !FieldArray)
		...
		for (const TSharedPtr<FJsonValue>& Value : *FieldArray)
		{
			...
			T& NewItem = Destination.Emplace_GetRef();
			SerializeAction(NewItem, Value->AsObject());
		}
