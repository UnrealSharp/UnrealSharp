...
public static class StringBuilderExtensions
{
    ...
    public static void BeginGeneratedSourceFile(this GeneratorStringBuilder builder, UnrealType type)
    {
        ...
        if (!string.IsNullOrEmpty(type.FieldName.Namespace))
        {
            builder.AppendLine($"namespace {type.FieldName.Namespace};");
            builder.AppendLine();
        }
    }
    ...
    {
        string json = JsonUtilities.SerializeType(type);

        string safeName = MakeIdentifier(type.FieldName.FullName);

        string registrarClassName = $"{safeName}_Registration";
        string jsonPropertyName = $"ReflectionMetadata_{safeName}";
        string registrationMethodName = $"Register_{safeName}";
        ...
        builder.AppendLine($"static string {jsonPropertyName} => \"\"\"\n{json}\n\"\"\";");

        builder.CloseBrace();
    }

    private static string MakeIdentifier(string fullName)
    {
        System.Text.StringBuilder builder = new System.Text.StringBuilder(fullName.Length);
    ...
    {
        BeginTypeStaticConstructor(builder, unrealType.FieldName.SourceName);
    }
    ...
    public static void AllocateParameterBuffer(this GeneratorStringBuilder builder, string sizeName)
    {
        ...
        builder.AppendLine(
            $"nint {SourceGenUtilities.ParamsBuffer} = (nint) {SourceGenUtilities.ParamsBufferAllocation};");
    }
